...
void Mesh::AddQuad(Math::Direction orientation, glm::vec3 offset, float uvScale, glm::vec2 uvOffset, unsigned char ambients[])
{
	...
	{
		const size_t current = vertices_.size() - (Math::CORNER_COUNT - i);
	...
	indexCount_ += static_cast<GLsizei>(std::size(quadIndices));
}
...
void Mesh::SetupObjects(size_t reserve)
{
	...
	// ambient
	glVertexAttribPointer(3, 1, GL_UNSIGNED_BYTE, GL_FALSE, sizeof(Vertex), reinterpret_cast<void*>(offsetof(Vertex, ambient)));
