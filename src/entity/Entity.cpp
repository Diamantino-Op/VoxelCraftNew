...
void Entity::Move(glm::vec3 delta)
{
	...
	for (std::size_t i = 0; i < std::size(axes); i++)
	{
		...
		{
			const float sign = glm::sign(delta[axes[i]]); // direction
			const float edge = position_[axes[i]] + sign * size_[axes[i]] / 2.0f; // entity edge offset

			const float distToNext = Math::DistToBlock(edge, static_cast<Math::Axis>(axes[i]), sign == -1.0f); // distance to next block
			const float offset = sign * glm::min(distToNext, glm::abs(delta[axes[i]])); // distance to move
			...
			else
			{
				...
				OnCollision(std::make_pair(collisions, Math::AxisToDir(static_cast<Math::Axis>(axes[i]), sign == -1.f)));
				break;
			}
		}
	}
