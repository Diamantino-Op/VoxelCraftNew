...
void Player::Update(float dt)
{
	...
	// Build
	const bool placing = fastPlace ? input.GetKey(GLFW_MOUSE_BUTTON_RIGHT) : input.GetKeyPressed(GLFW_MOUSE_BUTTON_RIGHT);
	const bool destroying = fastPlace ? input.GetKey(GLFW_MOUSE_BUTTON_LEFT) : input.GetKeyPressed(GLFW_MOUSE_BUTTON_LEFT);
	if (placing != destroying)
	{
		// Perform raycast
		const ChunkManager::RaycastResult raycast = chunk.Raycast(camera_.GetPosition(), camera_.GetForward(), INFINITY);
	...
	// Look
	const glm::vec2 deltaMouse = input.GetDeltaMouse() * 0.001f;
	...
	else // noclip
	{
		...
		if (dir != glm::vec3(0.0f))
			Teleport(GetPosition() + normalize(dir) * (input.GetKey(GLFW_KEY_LEFT_SHIFT) ? 100.f : 10.f) * dt);
	}

	// Update camera
	camera_.SetPosition(GetPosition() + glm::vec3(0.0f, GetSize().y * 0.4f, 0.0f));
	camera_.SetAspect(static_cast<float>(WindowManager::Instance().GetResolution().x) / static_cast<float>(WindowManager::Instance().GetResolution().y));
}
