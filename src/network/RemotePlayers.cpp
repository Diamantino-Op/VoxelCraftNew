...
bool RemoteHost::Update(const PlayerPacket &packet, const std::vector<BlockUpdate> &blocks)
{
    ...
    for (;;)
    {
        ...
        const int result = udp_.Receive(&pack, sizeof(pack));
    ...
    // Send packets
    udp_.Send(tcp_.GetRemoteAddress(), &packet, static_cast<int>(sizeof(packet)));
    tcp_.Send(blocks);
...
bool RemoteClients::Update(const PlayerPacket &packet, const std::vector<BlockUpdate> &blocks)
{
    ...
    for (;;)
    {
        ...
        const int result = udp_.Receive(&pack, sizeof(pack), address);
        ...
        // Create/lookup index
        const uint8_t index = static_cast<uint8_t>(addressMap_.insert(std::make_pair(address, addressMap_.size())).first->second);
    ...
    for (const auto &address : addressMap_)
    {
        ...
        for (size_t i = 0; i < players_.size(); i++)
        {
            ...
            IdentifiedPlayerPacket clientPacket = { static_cast<uint8_t>(++id), players_[i] };
